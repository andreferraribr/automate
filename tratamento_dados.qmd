...
anexo_03_receita %>% regex_inner_join(linhas_anexo_03, by = c("linhas_clean"))
```

`{r}data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACgAAAAaCAYAAADFTB7LAAAAa0lEQVR42u3OywnAIBBAwcXSUoCW5D11xDoNCBGNv0MOecJOBSOi1OZMsJ4dvFxEJ1OQnMxBarIKEpNNkJbsBknJYZCSnAYJyVVQziNig7/nZkFEbhTE5HpBVO4dxOXKIDL3BLG5BJ1T6rsbMfep2CaMN00AAAAASUVORK5CYII=} library(bslib) library(DT) library(dplyr) library(bsicons) dados_receita %>%   clean_names() %>%   filter(esfera_orcamentaria_codigo == '2') %>%   group_by(unidade_orcamentaria_codigo, unidade_orcamentaria_nome) %>%   summarise(across(c(receita_orcamentaria_liquida), sum, na.rm = TRUE))`
