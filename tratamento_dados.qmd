...
criar_codigo_filtros_linhas<- criar_codigo_filtros_linhas  %>% pivot_wider(names_from = sequencial, values_from = filtro) 
 
 # colunas_com_na <- criar_codigo_filtros_gerais_anexos_tabelas %>% keep(~any(is.na(.x))) %>% names
 
 # criar_codigo_filtros_gerais_anexos_tabelas <- criar_codigo_filtros_gerais_anexos_tabelas %>% mutate_all(colunas_com_na)

criar_codigo_filtros_linhas <- criar_codigo_filtros_linhas %>% unite(col= filtrao,  !c("anexo_tabela", "topico", "item","n"), sep = ' ') %>% mutate (filtrao = str_remove(filtrao, " NA"))
