...
dados_receita <- read_excel("auto_receita.xlsx")
...
dados_receita <- dados_receita %>% clean_names()
```
...
criar_codigo_filtros_gerais_anexos_tabelas <- filtros_gerais_anexos_tabelas %>%mutate(operador_logico_r = str_replace_all(operador_logico_r, "NA", " "))  %>% mutate(filtro = str_c(operador_logico_r, " ",atributo_r," " ,operador_comparacao_r, " " ,valores)) %>% group_by(anexo_tabela, sequencial,filtro)  %>% count() %>% drop_na()


 criar_codigo_filtros_gerais_anexos_tabelas <- criar_codigo_filtros_gerais_anexos_tabelas  %>% pivot_wider(names_from = sequencial, values_from = filtro) 
 
 # colunas_com_na <- criar_codigo_filtros_gerais_anexos_tabelas %>% keep(~any(is.na(.x))) %>% names
 
 # criar_codigo_filtros_gerais_anexos_tabelas <- criar_codigo_filtros_gerais_anexos_tabelas %>% mutate_all(colunas_com_na)

jjj <- criar_codigo_filtros_gerais_anexos_tabelas %>% unite(col= filtrao,  !c("anexo_tabela", "n"), sep = ' ') %>% mutate (filtrao = str_remove(filtrao, " NA"))

jjj <- jjj %>% mutate(filtrao = str_c(anexo_tabela, " <- dados_receita %>%  filter( ", filtrao, " )" )) %>% mutate (filtrao = str_remove(filtrao, " NA"))
...
criar_codigo_filtros_gerais_anexos_tabelas <- filtros_gerais_anexos_tabelas %>%
  ...
  mutate(filtro = 
           str_c(operador_logico_r,
                 ...
                 " " ,
                 valores)) %>% 
  group_by(anexo_tabela, sequencial,filtro)  %>%
  count() %>%
  drop_na()
...
#| eval: false
lll <- criar_codigo_filtros_gerais_anexos_tabelas %>% filter(anexo_tabela == "anexo_04_rgps")
...
criar_codigo_filtros_gerais_anexos_tabelas_linhas <- filtros_linhas %>%mutate(operador_logico_r = str_replace_all(operador_logico_r, "NA", " "))  %>% mutate(filtro = str_c(operador_logico_r, " ",atributo_r," " ,operador_comparacao_r, " " ,valores)) %>% group_by(anexo_tabela, topico, item, sequencial,filtro)  %>% count() %>% drop_na() %>% mutate(item = item %>% make_clean_names())


 criar_codigo_filtros_gerais_anexos_tabelas_linhas<- criar_codigo_filtros_gerais_anexos_tabelas_linhas  %>% pivot_wider(names_from = sequencial, values_from = filtro) 
 
 # colunas_com_na <- criar_codigo_filtros_gerais_anexos_tabelas %>% keep(~any(is.na(.x))) %>% names
 
 # criar_codigo_filtros_gerais_anexos_tabelas <- criar_codigo_filtros_gerais_anexos_tabelas %>% mutate_all(colunas_com_na)

jjj_linhas <- criar_codigo_filtros_gerais_anexos_tabelas_linhas %>% unite(col= filtrao,  !c("anexo_tabela", "topico", "item","n"), sep = ' ') %>% mutate (filtrao = str_remove(filtrao, " NA"))
