...
execute:
  warning: false
  message: false
  cache: true  # ⚡ CACHE GLOBAL ATIVADO
format:
  html:
    theme: cosmo
    toc: true
...
# ============================================================================
# SISTEMA DE CONTROLE DE EXECUÇÃO POR RELATÓRIOS
# ============================================================================
params:
  # === RELATÓRIOS A GERAR ===
  relatorios_ativos:
    ...
    choices: [
      ...
      "RGF_A02",  # Dívida Consolidada Líquida (DCL)
      "RGF_A03",  # Garantias e Contragarantias
      "RGF_A04",  # Operações de Crédito
      "RGF_A05",  # Disponibilidades de Caixa (União)
      "RGF_T04",  # Tabela 04 - Disponibilidade de Caixa e RAP
      # RREO - Relatório Resumido da Execução Orçamentária
      "RREO_A01", # Balanço Orçamentário
      "RREO_A02", # Demonstrativo por Função/Subfunção
      "RREO_A03", # Receita Corrente Líquida (RCL)
      "RREO_A04", # Receitas e Despesas Previdenciárias
      "RREO_A06", # Resultado Primário
      "RREO_A07", # Restos a Pagar
      "RREO_A08", # Receitas e Despesas com MDE
      "RREO_A09", # Demonstrativo da Regra de Ouro
      "RREO_A11", # Regime Próprio de Previdência
      "RREO_A12", # Receitas e Despesas com Saúde
      "RREO_T01", # Tabela 01 - Receitas Orçamentárias
      "RREO_T02", # Tabela 02 - Seguridade Social
      "RREO_T03", # Tabela 03 - Consolidação
      "RREO_T04"  # Tabela 04 - União e Executivo
    ]
  
  # === CONTROLE DE DEPENDÊNCIAS ===
  incluir_dependencias:
    value: true
    label: "Incluir dependências automaticamente"
...
DEPENDENCIAS <- list(
  # ==========================================================================
  # RGF - Todos os anexos dependem da RCL para cálculo de limites LRF
  # ==========================================================================
  "RGF_A01" = c("RREO_A03"),  # Despesa Pessoal → precisa RCL
  "RGF_A02" = c("RREO_A03"),  # DCL → precisa RCL
  "RGF_A03" = c("RREO_A03"),  # Garantias → precisa RCL
  "RGF_A04" = c("RREO_A03"),  # Operações de Crédito → precisa RCL
  "RGF_A05" = c("RREO_A03"),  # Disponibilidades União → precisa RCL
  "RGF_T04" = c(),             # Tabela 04 → independente
  
  # ==========================================================================
  # RREO
  # ==========================================================================
  "RREO_A01" = c(),            # Balanço Orçamentário → independente
  "RREO_A02" = c(),            # Função/Subfunção → independente
  "RREO_A03" = c(),            # RCL → base para RGF e outros RREO
  "RREO_A04" = c("RREO_A03"),  # Previdência → precisa RCL
  "RREO_A06" = c("RGF_A02"),   # Resultado Primário → precisa DCL
  "RREO_A07" = c(),            # Restos a Pagar → independente
  "RREO_A08" = c(),            # MDE → usa RLI (não RCL), independente
  "RREO_A09" = c(),            # Regra de Ouro → independente
  "RREO_A11" = c(),            # Regime Próprio → independente
  "RREO_A12" = c("RREO_A03"),  # Saúde → precisa RCL para % mínimo
  "RREO_T01" = c(),            # Tabela 01 → independente
  "RREO_T02" = c(),            # Tabela 02 → independente
  "RREO_T03" = c(),            # Tabela 03 → independente
  "RREO_T04" = c()             # Tabela 04 → independente (não depende DCL)
)

# ============================================================================
# FUNÇÃO DE RESOLUÇÃO DE DEPENDÊNCIAS
# ============================================================================
...
resolver_dependencias <- function(relatorios_solicitados, incluir_deps = TRUE) {
  ...
  # Loop até não haver mais dependências para adicionar
  while (TRUE) {
    novos <- c()
...
# Aplicar resolução de dependências
...
# ============================================================================
# MAPEAMENTO DE DADOS NECESSÁRIOS POR RELATÓRIO
# ============================================================================
...
cat("\n")
cat("╔════════════════════════════════════════════════════════════════╗\n")
cat("║       SISTEMA DE RELATÓRIOS FISCAIS - CONTROLE DE EXECUÇÃO     ║\n")
cat("╠════════════════════════════════════════════════════════════════╣\n")
cat(sprintf("║ Período: %-54s ║\n", MES_REF))
cat("╠════════════════════════════════════════════════════════════════╣\n")

# Mostrar relatórios solicitados
cat("║ 📋 RELATÓRIOS SOLICITADOS:                                     ║\n")
for (rel in RELATORIOS_SOLICITADOS) {
  # Obter nome descritivo
  nome_desc <- switch(rel,
    "RGF_A01" = "RGF Anexo 01 - Despesa com Pessoal",
    "RGF_A02" = "RGF Anexo 02 - Dívida Consolidada Líquida",
    "RGF_A03" = "RGF Anexo 03 - Garantias e Contragarantias",
    "RGF_A04" = "RGF Anexo 04 - Operações de Crédito",
    "RGF_A05" = "RGF Anexo 05 - Disponibilidades (União)",
    "RGF_T04" = "RGF Tabela 04 - Disponibilidade e RAP",
    "RREO_A01" = "RREO Anexo 01 - Balanço Orçamentário",
    "RREO_A02" = "RREO Anexo 02 - Função/Subfunção",
    "RREO_A03" = "RREO Anexo 03 - RCL",
    "RREO_A04" = "RREO Anexo 04 - Previdência",
    "RREO_A06" = "RREO Anexo 06 - Resultado Primário",
    "RREO_A07" = "RREO Anexo 07 - Restos a Pagar",
    "RREO_A08" = "RREO Anexo 08 - MDE",
    "RREO_A09" = "RREO Anexo 09 - Regra de Ouro",
    "RREO_A11" = "RREO Anexo 11 - Regime Próprio",
    "RREO_A12" = "RREO Anexo 12 - Saúde",
    "RREO_T01" = "RREO Tabela 01 - Receitas",
    "RREO_T02" = "RREO Tabela 02 - Seguridade",
    "RREO_T03" = "RREO Tabela 03 - Consolidação",
    "RREO_T04" = "RREO Tabela 04 - União/Executivo",
    rel
  )
  cat(sprintf("║   ✓ %-58s ║\n", nome_desc))
}

# Mostrar dependências adicionadas
if (INCLUIR_DEPENDENCIAS) {
  ...
  if (length(dependencias_adicionadas) > 0) {
    cat("╠════════════════════════════════════════════════════════════════╣\n")
    cat("║ 🔗 DEPENDÊNCIAS ADICIONADAS AUTOMATICAMENTE:                   ║\n")
    for (dep in dependencias_adicionadas) {
      nome_desc <- switch(dep,
        "RREO_A03" = "RREO Anexo 03 - RCL (para cálculo de limites)",
        "RGF_A02" = "RGF Anexo 02 - DCL (para resultado primário)",
        dep
      )
      cat(sprintf("║   + %-58s ║\n", nome_desc))
    }
...
# ============================================================================
# Esta variável muda quando os parâmetros mudam, invalidando o cache
...
# AVISO SOBRE CACHE
# ============================================================================
cat("💡 DICA: Se os resultados não mudarem após alterar os parâmetros,\n")
...
cat("📥 Importando dados RCL (RREO A03)...\n")
...
cat(sprintf("✓ RCL Despesas: %s registros\n", format(nrow(rreo_a03_rcl_despesas), big.mark = ".")))
cat(sprintf("✓ RCL Receitas: %s registros\n", format(nrow(rreo_a03_rcl_receitas), big.mark = ".")))
```

## 👥 Dados RGF Anexo 01 - Despesa com Pessoal
...
cat("📥 Importando dados RGF A01 (Despesa Pessoal)...\n")
...
cat(sprintf("✓ RGF A01: %s registros (%d arquivos)\n", format(nrow(rgf_a01), big.mark = "."), length(arq_rgf_a01)))
```

## 🏛️ Dados RGF Anexo 02 - Dívida Consolidada Líquida (DCL)
...
cat("📥 Importando dados RGF A02 (DCL)...\n")
...
cat(sprintf("✓ RGF A02: %d arquivos importados\n", length(arq_rgf_a02)))
```

## 🔐 Dados RGF Anexo 03 - Garantias e Contragarantias
...
cat("📥 Importando dados RGF A03 (Garantias)...\n")
...
cat(sprintf("✓ RGF A03: %s registros\n", format(nrow(rgf_a03), big.mark = ".")))
```

## 💳 Dados RGF Anexo 04 - Operações de Crédito
...
cat("📥 Importando dados RGF A04 (Operações Crédito)...\n")

# RGF Anexo 4 - Receitas
...
cat(sprintf("✓ RGF A04 Receitas: %s registros\n", format(nrow(rgf_a04_receitas), big.mark = ".")))
cat(sprintf("✓ RGF A04 Despesas: %s registros\n", format(nrow(rgf_a04_despesas), big.mark = ".")))
```

## 💰 Dados RGF Anexo 05 - Disponibilidades de Caixa
...
cat("📥 Importando dados RGF A05 (Disponibilidades)...\n")
...
cat(sprintf("✓ RGF A05: %s registros (%d arquivos)\n", format(nrow(rgf_a05), big.mark = "."), length(arq_rgf_a05)))
```
...
consolidar_criterios <- function() {
  ...
  # Identificar colunas disponíveis para agrupamento
...
cat("\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("   RGF - RELATÓRIO DE GESTÃO FISCAL\n")
...
criterios_rreo_A03_rcl_receitas_rcl <- list(
  ...
  # DEDUÇÕES DAS RECEITAS - CORRIGIDO
...
divida <- resultado$variacao[resultado$item == "Subconta da Dívida"]
...
regra_ouro_df <- data.frame(
  ...
  Valor = regra_ouro,
  Status = ifelse(regra_ouro > 0, "NÃO CUMPRIDA", "CUMPRIDA")
)

# Exibir tabelas com DT::datatable
datatable(resultado, 
          caption = paste("Variação dos Saldos - Mês", mes_atual, "vs Abertura"),
...
datatable(regra_ouro_df,
          caption = paste("Resultado da Regra de Ouro - Mês", mes_atual),
...
# 📥 Importação dos Dados {#sec-dados_2}
...
consolidar_criterios <- function() {
  ...
  tabelas_padronizadas <- map(tabelas, function(tabela) {
    ...
    # Lista de colunas que devem ser convertidas para character
    colunas_para_converter <- c(
      "item_informacao_codigo",
      "orgao_uge_tipo_administracao_codigo",
      "item_informacao_nome",
      "orgao_uge_tipo_administracao_nome"
    )
    
    # Converter apenas as colunas que existem
    for (coluna in colunas_para_converter) {
      if (coluna %in% names(tabela)) {
        tabela[[coluna]] <- as.character(tabela[[coluna]])
      }
    }
  ...
  # Identificar colunas disponíveis para agrupamento
  colunas_agrupamento <- c("mes_lancamento", "ordem", "nome", "metrica", "dataframe_nome", "dataframe_filtros",
                          "relatorio", "anexo_codigo", "anexo_nome", "detalhe", "print", "filtro")
  
  # Adicionar colunas opcionais se existirem
  colunas_opcionais <- c("item_informacao_nome", "item_informacao_codigo", 
                        "orgao_uge_tipo_administracao_nome", "orgao_uge_tipo_administracao_codigo")
  
  for (col in colunas_opcionais) {
    if (col %in% names(resultado_completo)) {
      colunas_agrupamento <- c(colunas_agrupamento, col)
    }
  }
  
  # Filtrar apenas colunas que realmente existem
  colunas_agrupamento <- colunas_agrupamento[colunas_agrupamento %in% names(resultado_completo)]
  ...
  # Reordenar colunas - apenas as que existem
  colunas_ordem <- c("chave", "mes_lancamento", "data_lancamento", "dataframe_nome", "dataframe_filtros", 
                    "relatorio", "anexo_codigo", "anexo_nome", "detalhe", "print", "filtro", "metrica")
  
  # Adicionar colunas opcionais se existirem
  for (col in colunas_opcionais) {
    if (col %in% names(resultado)) {
      colunas_ordem <- c(colunas_ordem, col)
    }
  }
...
cat("\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("   RREO - RELATÓRIO RESUMIDO DA EXECUÇÃO ORÇAMENTÁRIA\n")
cat(sprintf("   Período: %s\n", MES_REF))
cat("═══════════════════════════════════════════════════════════════\n")
cat("\n")
```
...
bo_depsesa <- (aplicar_criterios(dados_despesa%>% filter(item_informacao_codigo == "23"), criterios_rreo_A01_despesas_bo))
...
```{r resuldado_rgps}
formatar_numero(resultado_rgps <- (
  aplicar_criterios(df = dados_receita %>% 
    filter(
      unidade_orcamentaria_codigo %in% c(33904, 40904, 55902, 25917) | 
      natureza_receita_codigo_completo == 79900211,
           item_informacao_codigo %in% c(5)
      ), criterios_rreo_A04_rgps_receitas_rgps) %>% filter(ordem != "02") %>% summarise(valor=sum(valor))
  -
    ...
    filter(unidade_orcamentaria_codigo %in% c(33904, 40904, 55902, 25917),
           ...
           ), criterios_rreo_A04_rgps_despesas_rgps)%>% summarise(valor=sum(valor))
))
