...
# Carregar dependências JavaScript para botões
htmltools::tagList(
  ...
  tags$link(rel = "stylesheet", href = "https://cdn.datatables.net/buttons/2.4.2/css/buttons.dataTables.min.css")
)
...
datatable(aplicar_criterios(rgf_a03, criterios_rgf_A03_garantias) %>% group_by(ordem,nome) %>% summarise(valor = sum(valor)),
  ...
  options = list(
    ...
    lengthMenu = list(c(10, 25, 50, -1), c('10', '25', '50', 'Todas')),
    columnDefs = list(
      list(visible = FALSE, targets = 0)  # Oculta a primeira coluna (ordem)
    )
  ))%>% 
