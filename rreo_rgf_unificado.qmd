...
dados_receita <- read_excel("data/tg_julho/tg_receita_julho_2025.xlsx") %>% 
  clean_names() %>% 
  filter(orgao_uge_orcam_fiscal_s_n == "PERTENCE")
dados_receita$mes_lancamento %>% unique() %>% fct_inorder()
...
arq_despesa <- list.files(path = "data/tg_julho/", pattern = 'tg_despesa', full.names = TRUE)

dados_despesa <- map_df(arq_despesa, read_excel) %>%
  clean_names() %>%
  filter(orgao_uge_orcam_fiscal_s_n == "PERTENCE")
dados_despesa$mes_lancamento %>% unique() %>% fct_inorder() 
...
# RP Anexo 07
dados_rp_anexo_07 <- read_excel("data/tg_julho/tg_rp_anexo_07_julho_2025.xlsx") %>% 
  clean_names()
...
dados_rp_anexo_07$mes_lancamento %>% unique() %>% fct_inorder() 

# RP Anexo 12
dados_rp_anexo_12 <- read_excel("data/tg_julho/tg_rp_anexo_12_julho_2025.xlsx") %>% 
  clean_names()

# RP Anexo 08
dados_rp_anexo_08 <- read_excel("data/tg_julho/tg_rp_anexo_08_julho_2025.xlsx") %>% 
  clean_names()


dados_rp_anexo_03_rcl <- read_excel("data/tg_julho/tg_rp_anexo_03_rcl_julho_2025.xlsx") %>% 
  clean_names()
dados_rp_anexo_03_rcl$mes_lancamento %>% unique() %>% fct_inorder() 
...
# Buscar todos os arquivos de despesa
arq_tabela_04<- list.files(path = "data/tg_julho/", pattern = 'tabela_04', full.names = TRUE)

dados_tabela_04 <- map_df(arq_tabela_04, read_excel) %>%
  clean_names() 



# Dados específicos da UG 170600
# dados_tabela_04_170600 <- read_excel("data/tg_julho/dados_conta_contabil_tabela_04_claude_conta_contabil_170600.xlsx") %>% 
#   clean_names() %>% 
#   mutate(ug = "170600")

# Dados filtro RREO - demais UGs
dados_tabela_04_filtro_rreo <- read_excel("data/tg_julho/tg_rreo_tabela_04_filtro_rreo_julho_2025.xlsx") %>% 
  clean_names() %>% 
  mutate(ug = "demais")
...
dados_ndd <- read_excel("data/tg_julho/tg_anexo_06_rreo_despesas_primarias_ndd_julho_2025.xlsx") %>% 
  clean_names()

# Dados Categoria/Grupo
dados_categoria_grupo <- read_excel("data/tg_julho/tg_anexo_06_rreo_despesas_primarias_categoria_grupo_julho_2025.xlsx") %>% 
  clean_names()
...
# RPN - Juros
tg_RPN_juros <- read_excel("data/tg_julho/tg_RPN_juros_julho_2025.xlsx") %>% 
  clean_names()
dados_juros <- tg_RPN_juros
dados_juros$mes_lancamento %>% unique() %>% fct_inorder()
# RPN - Disponibilidades
tg_RPN_disponibilidades <- read_excel("data/tg_julho/tg_RPN_disponibilidades_julho_2025.xlsx") %>% 
  clean_names()
tg_RPN_disponibilidades$mes_lancamento %>% unique() %>% fct_inorder()

dados_ndd <- read_excel("data/tg_julho/tg_anexo_06_rreo_despesas_primarias_ndd_julho_2025.xlsx")%>% clean_names()

dcl_1 <- read_excel("data/tg_julho/dcl_1_julho_2025.xlsx") %>% clean_names()


dcl_2a <- read_excel("data/tg_julho/dcl_2a_julho_2025.xlsx") %>% clean_names()



arq_dcl_2b<- list.files(path = "data/tg_julho/", pattern = 'dcl_2b', full.names = TRUE)

dcl_2b <- map_df(arq_dcl_2b, read_excel) %>%
  clean_names() 
...
processar_dt_criterios_conta_contabil <- function(df) {
  df %>% 
    separate(categoria, into = c("ordem", "nome"), sep = 2, remove = FALSE) %>% 
    filter(mes_lancamento == mes_filtro) %>%
    group_by(ordem, nome, categoria) %>% 
    summarise(saldo = sum(saldo_r_conta_contabil, na.rm = TRUE), .groups = "drop") %>%
    complete(categoria, fill = list(saldo = 0)) 
}
# Exemplo de uso:
...
agrupar_e_combinar_dados_conta_contabil <- function(df, criterios, metrica = "saldo_r_conta_contabil") {
...
```{r}
datatable(dados_despesa %>% filter(mes_lancamento == mes_filtro, tipo_modalidade != "intra", item_informacao_nome == "DESPESAS EMPENHADAS") %>% group_by(refinanciamento, grupo_despesa_codigo_grupo, grupo_despesa_nome) %>% summarise(despesa_empenhada = sum(saldo_r_item_informacao, na.rm = TRUE) )  %>% adorn_totals("row"), rownames = FALSE,
...
datatable(
  ...
  caption = "receitas_anexo3_rcl"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios( receitas_anexo3_rcl), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
...
```{r rcl_despesa}
datatable(
   processar_dt_criterios( despesas_anexo3_rcl),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "despesas_anexo3_rcl"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios( despesas_anexo3_rcl), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
...
formatar_numero((sum(receitas_anexo3_rcl$saldo_r_item_informacao)-sum(despesas_anexo3_rcl$saldo_r_item_informacao)- sum(rp_cancelados_anexo3_rcl$saldo_r_item_informacao))/1000)
...
# ===============================================================================
criterios_receitas_primarias_rreo <- list(
...
# ===============================================================================
criterios_despesas_categoria_grupo_rreo <- list(
...
# ===============================================================================
criterios_despesas_natureza_detalhada_rreo <- list(
...
# ===============================================================================
criterios_transferencias_subsidios_rreo <- list(
...
# ===============================================================================
criterios_disponibilidades_rreo <- list(
...
# ===============================================================================
criterios_juros_ativos_passivos_rreo <- list(
...
```{r processar_resultado_primario}
 ...
 despesas_categoria_grupo_rreo <- agrupar_e_combinar_dados(
       df =  dados_categoria_grupo %>% filter(mes_lancamento == mes_filtro),
        criterios = criterios_despesas_categoria_grupo_rreo
      )
 
 datatable(
  processar_dt_criterios( despesas_categoria_grupo_rreo),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "despesas_categoria_grupo_rreo"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios( despesas_categoria_grupo_rreo), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
  ...
  receitas_primarias_rreo <- agrupar_e_combinar_dados(
       df =  dados_receita,
        criterios = criterios_receitas_primarias_rreo
      )
 
 datatable(
  processar_dt_criterios( receitas_primarias_rreo),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "eceitas_primarias_rreo"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios( receitas_primarias_rreo), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
...
despesas_natureza_detalhada_rreo <-   agrupar_e_combinar_dados(
       df =  dados_ndd,
        criterios = criterios_despesas_natureza_detalhada_rreo )


 datatable(
  processar_dt_criterios( despesas_natureza_detalhada_rreo),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "despesas_natureza_detalhada_rreo"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios( despesas_natureza_detalhada_rreo), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
...
juros_ativos_passivos_rreo <- agrupar_e_combinar_dados_conta_contabil(
       df =  dados_juros,
        criterios = criterios_juros_ativos_passivos_rreo
      ) 
...
datatable(
  juros_ativos_passivos_rreo %>% filter(mes_lancamento == mes_filtro) %>% pivot_wider(names_from = mes_lancamento, values_from = saldo_r_conta_contabil),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "juros_ativos_passivos_rreo"
) %>%
  formatRound(
    mes_filtro, 
    digits = 2, 
  ...
  disponibilidades_rreo <- agrupar_e_combinar_dados_conta_contabil( 
    df =  tg_RPN_disponibilidades,
        criterios =criterios_disponibilidades_rreo)
  
  
  datatable(
  disponibilidades_rreo %>% filter(mes_lancamento == mes_filtro) %>% pivot_wider(names_from = mes_lancamento, values_from = saldo_r_conta_contabil),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "disponibilidades_rreo"
) %>%
  formatRound(
    mes_filtro, 
    digits = 2, 
...
```{r anexo_06_receitas_primarias}
# RREO ANEXO 6 - RECEITAS PRIMÁRIAS (CÓDIGO SIMPLES)
# Replica exatamente os valores da planilha oficial
...
# =============================================================================
# FUNÇÃO SIMPLES PARA RECEITAS PRIMÁRIAS
# =============================================================================

receitas_primarias_rreo <- function(dados_receita) {
    
    # Filtros conforme critérios do Tesouro Gerencial
    dados <- dados_receita %>%
        filter(
            # {NRE1 Categoria Econômica} <> 7:RECEITAS CORRENTES, 8:RECEITAS DE CAPITAL
            nre1_categoria_economica_codigo %in% c(1, 2),  # 1:RECEITAS CORRENTES, 2:RECEITAS DE CAPITAL
            # {Órgão UGE - Orçam. Fiscal S/N} = PERTENCE
            orgao_uge_orcam_fiscal_s_n == "PERTENCE",
            # Filtro para receita líquida para maior precisão
            item_informacao_codigo == 5  # Receita Líquida
        )
    
    # Função para ordenar meses cronologicamente
    ordenar_meses <- function(meses_lista) {
        ordem_meses <- c("JAN", "FEV", "MAR", "ABR", "MAI", "JUN", 
                        "JUL", "AGO", "SET", "OUT", "NOV", "DEZ")
        
        meses_df <- data.frame(
            mes_completo = meses_lista,
            mes = substr(meses_lista, 1, 3),
            ano = substr(meses_lista, 5, 8),
            stringsAsFactors = FALSE
        )
        
        meses_df$ordem_mes <- match(meses_df$mes, ordem_meses)
        meses_ordenados <- meses_df[order(meses_df$ano, meses_df$ordem_mes), ]
        
        return(meses_ordenados$mes_completo)
    }
    
    # Obter meses disponíveis e ordenar
    meses_ordenados <- ordenar_meses(unique(dados$mes_lancamento))
    
    # Criar estrutura final do relatório
    relatorio <- data.frame(
        elemento = c(
            "RECEITAS CORRENTES (I)",
            "___Impostos, Taxas e Contribuições de Melhoria", 
            "___Contribuições",
            "___Receita Patrimonial",
            "______Aplicações Financeiras (II)",
            "______Outras Receitas Patrimoniais",
            "___Transferências Correntes",
            "___Demais Receitas Correntes",
            "______Outras Receitas Financeiras (III)",
            "___Receitas Correntes Restantes",
            "RECEITAS PRIMÁRIAS CORRENTES (IV) = (I - II - III)",
            "RECEITAS DE CAPITAL (V)",
            "___Operações de Crédito (VI)",
            "___Amortização de Empréstimos (VII)",
            "___Alienações de Bens",
            "______Outras Alienações de Bens", 
            "___Transferências de Capital",
            "___Outras Receitas de Capital",
            "______Outras Receitas de Capital Não Primárias (X)",
    ...
    # Calcular valores para cada mês com filtros específicos
    for (mes in meses_ordenados) {
        
        dados_mes <- dados[dados$mes_lancamento == mes, ]
        
        # RECEITAS CORRENTES
        correntes_total <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1, "saldo_r_item_informacao"], na.rm = TRUE)
        
        impostos <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                dados_mes$nre2_origem_receita_codigo_origem == 1, "saldo_r_item_informacao"], na.rm = TRUE)
        
        contribuicoes <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                     dados_mes$nre2_origem_receita_codigo_origem == 2, "saldo_r_item_informacao"], na.rm = TRUE)
        
        patrimonial <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                   dados_mes$nre2_origem_receita_codigo_origem == 3, "saldo_r_item_informacao"], na.rm = TRUE)
        
        # APLICAÇÕES FINANCEIRAS - códigos específicos conforme prints
        aplicacoes <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                  (dados_mes$natureza_receita_codigo_completo %in% c("13210101", "13210201", "13210301", "13210501") |
                                   startsWith(dados_mes$natureza_receita_codigo_completo, "1321004") |
                                   startsWith(dados_mes$natureza_receita_codigo_completo, "1329001")), "saldo_r_item_informacao"], na.rm = TRUE)
        
        outras_patrimoniais <- patrimonial - aplicacoes
        
        transferencias <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                      dados_mes$nre2_origem_receita_codigo_origem == 7, "saldo_r_item_informacao"], na.rm = TRUE)
        
        # DEMAIS RECEITAS CORRENTES - apenas códigos conforme print (SEM exclusões)
        demais <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                              (startsWith(dados_mes$natureza_receita_codigo_completo, "14") |
                               startsWith(dados_mes$natureza_receita_codigo_completo, "15") |
                               startsWith(dados_mes$natureza_receita_codigo_completo, "16") |
                               startsWith(dados_mes$natureza_receita_codigo_completo, "18") |
                               startsWith(dados_mes$natureza_receita_codigo_completo, "19")), "saldo_r_item_informacao"], na.rm = TRUE)
        
        # OUTRAS RECEITAS FINANCEIRAS - códigos específicos conforme prints corretos
        outras_financeiras <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                          (dados_mes$natureza_receita_codigo_completo %in% c("16410101", "16410102", "19991101", "19991102", "16410301") |
                                           startsWith(dados_mes$natureza_receita_codigo_completo, "1922012") |
                                           startsWith(dados_mes$natureza_receita_codigo_completo, "1990992")), "saldo_r_item_informacao"], na.rm = TRUE)
        
        # RECEITAS CORRENTES RESTANTES - diferença entre Demais e Outras Financeiras
        receitas_correntes_restantes <- demais - outras_financeiras
        
        primarias_correntes <- correntes_total - aplicacoes - outras_financeiras
        
        # RECEITAS DE CAPITAL
        capital_total <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2, "saldo_r_item_informacao"], na.rm = TRUE)
        
        operacoes_credito <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                         dados_mes$nre2_origem_receita_codigo_origem == 1, "saldo_r_item_informacao"], na.rm = TRUE)
        
        amortizacao <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                   dados_mes$nre2_origem_receita_codigo_origem == 3, "saldo_r_item_informacao"], na.rm = TRUE)
        
        alienacoes <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                  dados_mes$nre2_origem_receita_codigo_origem == 2, "saldo_r_item_informacao"], na.rm = TRUE)
        
        outras_alienacoes <- alienacoes  # Para manter consistência com a planilha
        
        transf_capital <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                      dados_mes$nre2_origem_receita_codigo_origem == 4, "saldo_r_item_informacao"], na.rm = TRUE)
        
        outras_capital <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                      dados_mes$nre2_origem_receita_codigo_origem %in% c(5, 6, 7, 8, 9), "saldo_r_item_informacao"], na.rm = TRUE)
        
        # OUTRAS RECEITAS DE CAPITAL NÃO PRIMÁRIAS - códigos específicos conforme prints
        outras_capital_nao_primarias <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                                    (startsWith(dados_mes$natureza_receita_codigo_completo, "292") |
                                                     startsWith(dados_mes$natureza_receita_codigo_completo, "293") |
                                                     startsWith(dados_mes$natureza_receita_codigo_completo, "294")), "saldo_r_item_informacao"], na.rm = TRUE)
        
        primarias_capital <- capital_total - operacoes_credito - amortizacao - outras_capital_nao_primarias
        
        primaria_total <- primarias_correntes + primarias_capital
        
        # Adicionar valores ao relatório (manter como numeric para precisão)
        relatorio[[mes]] <- c(
            correntes_total,
            impostos,
            contribuicoes,
...
# Executar
relatorio_final <- receitas_primarias_rreo(dados_receita)

# Criar DT datatable com formatação em R$
criar_datatable_rreo <- function(relatorio) {
    
    # Identificar colunas de valores (excluir coluna 'elemento')
    colunas_valores <- names(relatorio)[names(relatorio) != "elemento"]
    ...
    dt <- DT::datatable(
        ...
        options = list(
            pageLength = 20,
            scrollX = TRUE,
            dom = 't',  # Apenas tabela, sem paginação
            columnDefs = list(
                list(className = 'dt-left', targets = 0),  # Alinhar primeira coluna à esquerda
                list(className = 'dt-right', targets = 1:length(colunas_valores))  # Alinhar números à direita
            )
        ),
        rownames = FALSE,
        class = 'cell-border stripe hover'
    ) %>%
    formatRound(which(sapply(relatorio, is.numeric)), 2, mark = ".", dec.mark = ",")
    
    return(dt)
}

# Gerar datatable formatado
datatable_rreo <- criar_datatable_rreo(relatorio_final)

# Exibir datatable
datatable_rreo

# Ver resultado simples também
# print(relatorio_final)

# Salvar em CSV se necessário
# write.csv2(relatorio_final, "receitas_primarias_rreo.csv", row.names = FALSE)
```

```{r anexo_06_despesas_primarias}
despesas_primarias_rreo <- function(dados_ndd, dados_categoria_grupo) {
    
    # FILTROS DIRETOS NAS DATAFRAMES
    
    # Lista de ações excluídas (subsídios)
    acoes_excluidas <- c('0021', '0029', '0030', '0031', '0061', '006A', '0427', 
                        '0534', '0A81', '0A84', '2130', '0062', '00DD', '00S5', 
                        '20GI', '006C', '00ED', '00EE', '00SG')
    ...
    dados_ndd <- dados_ndd %>%
        filter(item_informacao_codigo == 56,
          !acao_governo_codigo %in% acoes_excluidas,
               !is.na(mes_lancamento),
               mes_lancamento != "",
               # Remover linha com data do relatório TG (formato dd/mm/yyyy)
               !grepl("^\\d{1,2}/\\d{1,2}/\\d{4}$", mes_lancamento))
    
    # Filtrar dados_categoria_grupo  
    dados_categoria_grupo <- dados_categoria_grupo %>%
        filter(item_informacao_codigo == 56,
               !acao_governo_codigo %in% acoes_excluidas,
               !is.na(mes_lancamento), 
               mes_lancamento != "",
    ...
    # Obter meses únicos e ordenar cronologicamente
    meses_unicos <- unique(c(dados_ndd$mes_lancamento, dados_categoria_grupo$mes_lancamento))
    meses <- ordenar_meses(meses_unicos)
    
    # Estrutura do relatório
    relatorio <- data.frame(
        elemento = c(
            "DESPESAS CORRENTES (XIII)",
            "___Pessoal e Encargos Sociais", 
            "___Juros e Encargos da Dívida (XIV)",
            "___Outras Despesas Correntes",
            "______Transferências Constitucionais e Legais",
            "______Demais Outras Despesas Correntes", 
            "DESPESAS PRIMÁRIAS CORRENTES (XV) = (XIII - XIV)",
            "DESPESAS DE CAPITAL (XVI)",
            "___Investimentos",
            "___Inversões Financeiras", 
            "______Concessão de Empréstimos e Financiamentos (XVII)",
            "______Demais Inversões Financeiras",
            "______Amortização da Dívida (XX)",
            "DESPESAS PRIMÁRIAS DE CAPITAL (XXI) = (XVI - XVII - XX)",
            "DESPESA PRIMÁRIA TOTAL (XXII) = (XV + XXI)"
        ),
        stringsAsFactors = FALSE
    )
    ...
    for (mes in meses) {
        ...
        # Filtrar por mês
        ndd_mes <- dados_ndd[dados_ndd$mes_lancamento == mes, ]
        cat_mes <- dados_categoria_grupo[dados_categoria_grupo$mes_lancamento == mes, ]
        
        # DESPESAS CORRENTES (Categoria 3)
        correntes <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 3, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # PESSOAL (Categoria 3 + Grupo 1)
        pessoal <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 3 & cat_mes$grupo_despesa_codigo_grupo == 1, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # JUROS (Categoria 3 + Grupo 2)
        juros <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 3 & cat_mes$grupo_despesa_codigo_grupo == 2, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # OUTRAS DESPESAS CORRENTES (Categoria 3 + Grupo 3)
        outras_correntes <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 3 & cat_mes$grupo_despesa_codigo_grupo == 3, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # TRANSFERÊNCIAS CONSTITUCIONAIS (códigos específicos)
        transferencias <- sum(ndd_mes[startsWith(ndd_mes$natureza_despesa_detalhada_codigo, "333081") | startsWith(ndd_mes$natureza_despesa_detalhada_codigo, "334081"), "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # DEMAIS OUTRAS DESPESAS CORRENTES
        demais_correntes <- outras_correntes - transferencias
        
        # DESPESAS PRIMÁRIAS CORRENTES
        primarias_correntes <- correntes - juros
        
        # DESPESAS DE CAPITAL (Categoria 4)
        capital <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 4, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # INVESTIMENTOS (Categoria 4 + Grupo 4)
        investimentos <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 4 & cat_mes$grupo_despesa_codigo_grupo == 4, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # INVERSÕES FINANCEIRAS (Categoria 4 + Grupo 5)
        inversoes <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 4 & cat_mes$grupo_despesa_codigo_grupo == 5, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # CONCESSÃO DE EMPRÉSTIMOS (códigos específicos)
        concessao <- sum(ndd_mes[ndd_mes$natureza_despesa_detalhada_codigo %in% c("45909206", "45909208", "45909266"), "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # DEMAIS INVERSÕES FINANCEIRAS
        demais_inversoes <- inversoes - concessao
        
        # AMORTIZAÇÃO DA DÍVIDA (Categoria 4 + Grupo 6)
        amortizacao <- sum(cat_mes[cat_mes$categoria_economica_despesa_codigo == 4 & cat_mes$grupo_despesa_codigo_grupo == 6, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # DESPESAS PRIMÁRIAS DE CAPITAL
        primarias_capital <- capital - concessao - amortizacao
...
# =============================================================================
# FUNÇÃO PARA DATATABLE
# =============================================================================

criar_datatable_despesas <- function(relatorio) {
    ...
    dt <- DT::datatable(
        ...
        options = list(
            ...
            columnDefs = list(
                ...
                list(className = 'dt-right', targets = 1:length(colunas_valores))
            )
        ),
        rownames = FALSE,
        class = 'cell-border stripe hover'
...
# Para executar:
relatorio_despesas <- despesas_primarias_rreo(dados_ndd, dados_categoria_grupo)
datatable_despesas <- criar_datatable_despesas(relatorio_despesas)
datatable_despesas

# =============================================================================
# CAMPOS NECESSÁRIOS:
# - dados_ndd: mes_lancamento, acao_governo_codigo, natureza_despesa_detalhada_codigo, saldo_r_conta_contabil
# - dados_categoria_grupo: mes_lancamento, acao_governo_codigo, categoria_economica_despesa_codigo, grupo_despesa_codigo_grupo, saldo_r_conta_contabil
# =============================================================================
```

```{r anexo_06_rreo_juros}
# RREO ANEXO 6 - JUROS ATIVOS E PASSIVOS - CÓDIGO SIMPLIFICADO
# Totaliza apenas contas 34xxx (ativos) e 44xxx (passivos)



# =============================================================================
# FUNÇÃO SIMPLIFICADA PARA JUROS ATIVOS E PASSIVOS RREO
# =============================================================================

juros_ativos_passivos_rreo <- function(dados_juros) {
    
    # FILTROS DIRETOS NA DATAFRAME
    dados_juros <- dados_juros %>%
        filter(orgao_uge_orcam_fiscal_s_n == "PERTENCE",
               !is.na(mes_lancamento),
               mes_lancamento != "",
               # Remover célula com data de execução do relatório TG
               !grepl("^\\d{1,2}/\\d{1,2}/\\d{4}$", mes_lancamento),
               # Manter apenas meses válidos (formato MMM/YYYY)
               grepl("^[A-Z]{3}/\\d{4}$", mes_lancamento),
               # Manter apenas as contas específicas de juros (usando "começa com" e conta específica)
               (substr(conta_contabil_numero, 1, 5) %in% c(
                   # Juros Ativos (começa com)
                   "34111", "34113", "34114", "34115", "34121", "34131", 
                   "34141", "34181", "34183", "34184", "34185", "34191",
                   "34211", "34213", "34214", "34215", "34221", "34291",
                   # Juros Passivos (começa com)
                   "44111", "44113", "44114", "44115", "44121", "44131", 
                   "44133", "44134", "44135", "44141", "44211", "44213", 
                   "44214", "44215", "44221"
               )) |
               # Juros Passivos (contas específicas - exatas)
               conta_contabil_numero == "445110100" | conta_contabil_numero == "445210100")
    ...
    # Obter meses únicos e ordenar cronologicamente
    meses_unicos <- unique(dados_juros$mes_lancamento)
    meses <- ordenar_meses(meses_unicos)
    
    # Estrutura do relatório - COM FILTRO ORÇAMENTO FISCAL
    relatorio <- data.frame(
        elemento = c(
            "TOTAL JUROS ATIVOS (Orçamento Fiscal)",
            "TOTAL JUROS PASSIVOS (Orçamento Fiscal)",
            "RESULTADO LÍQUIDO DE JUROS (ATIVOS - PASSIVOS)",
            "FUNDOS - JUROS ATIVOS (Filtro Específico)",
            "FUNDOS - JUROS PASSIVOS (Filtro Específico)",
            "FUNDOS - RESULTADO LÍQUIDO"
        ),
        stringsAsFactors = FALSE
    )
    ...
    for (mes in meses) {
        ...
        # Filtrar por mês
        juros_mes <- dados_juros[dados_juros$mes_lancamento == mes, ]
        
        # === JUROS ATIVOS - CONTAS QUE COMEÇAM COM OS CÓDIGOS DA IMAGEM 1 ===
        contas_juros_ativos_prefixos <- c("44111", "44113", "44114", "44115", "44121", "44131", 
                                         "44133", "44134", "44135", "44141", "44211", "44213", 
                                         "44214", "44215", "44221")
        contas_juros_ativos_especificas <- c("445110100", "445210100")
        
        juros_ativos_filter <- 
            substr(juros_mes$conta_contabil_numero, 1, 5) %in% contas_juros_ativos_prefixos |
            juros_mes$conta_contabil_numero %in% contas_juros_ativos_especificas
        
        total_juros_ativos <- sum(juros_mes[juros_ativos_filter, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # === JUROS PASSIVOS - CONTAS QUE COMEÇAM COM OS CÓDIGOS DA IMAGEM 2 ===
        contas_juros_passivos_prefixos <- c("34111", "34113", "34114", "34115", "34121", "34131", 
                                           "34141", "34181", "34183", "34184", "34185", "34191",
                                           "34211", "34213", "34214", "34215", "34221", "34291")
        
        juros_passivos_filter <- substr(juros_mes$conta_contabil_numero, 1, 5) %in% contas_juros_passivos_prefixos
        total_juros_passivos <- sum(juros_mes[juros_passivos_filter, "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # Resultado Líquido
        resultado_liquido <- total_juros_ativos - total_juros_passivos
        
        # === FILTRO ESPECÍFICO PARA FUNDOS ===
        fundos_mes <- juros_mes %>%
            filter(
                # Tipo Administração = 7 (FUNDOS)
                orgao_uge_tipo_administracao_codigo == "7",
                # MANTER: Apenas órgãos que pertencem ao orçamento fiscal
                orgao_uge_orcam_fiscal_s_n == "PERTENCE",
                # UG Executora diferente de 380916 (CGFAT)
                ug_executora_codigo != "380916",
                # Órgão UGE diferente de 25915 e 37904
                !orgao_uge_codigo %in% c("25915", "37904")
            )
        
        # Fundos - Juros Ativos (usando mesma lógica)
        fundos_juros_ativos <- sum(fundos_mes[
            substr(fundos_mes$conta_contabil_numero, 1, 5) %in% contas_juros_ativos_prefixos |
            fundos_mes$conta_contabil_numero %in% contas_juros_ativos_especificas, 
            "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # Fundos - Juros Passivos (usando mesma lógica)
        fundos_juros_passivos <- sum(fundos_mes[
            substr(fundos_mes$conta_contabil_numero, 1, 5) %in% contas_juros_passivos_prefixos, 
            "saldo_r_conta_contabil"], na.rm = TRUE)
        
        # Fundos - Resultado Líquido
        fundos_resultado_liquido <- fundos_juros_ativos - fundos_juros_passivos
        
        # Adicionar ao relatório
        relatorio[[mes]] <- c(
            total_juros_ativos,
            total_juros_passivos,
            resultado_liquido,
            fundos_juros_ativos,
            fundos_juros_passivos,
            fundos_resultado_liquido
        )
    }
...
criar_datatable_juros <- function(relatorio) {
    
    # Identificar colunas com valores numéricos
    colunas_valores <- names(relatorio)[names(relatorio) != "elemento"]
    ...
    # Aplicar formatação apenas em colunas numéricas não vazias
    for (i in 2:ncol(relatorio)) {
        if (is.numeric(relatorio[[i]])) {
            dt <- dt %>% formatRound(i, 2, mark = ".", dec.mark = ",")
        }
    }
...
# Para executar o relatório principal:
# relatorio_juros <- juros_ativos_passivos_rreo(tg_RPN_juros)
# datatable_juros <- criar_datatable_juros(relatorio_juros)
# datatable_juros

# =============================================================================
# CAMPOS NECESSÁRIOS NA DATAFRAME tg_RPN_juros:
# - mes_lancamento: Mês de referência (ex: "JAN/2025")
# - orgao_uge_orcam_fiscal_s_n: Indica se pertence ao orçamento fiscal ("PERTENCE")
# - saldo_r_conta_contabil: Valor em R$
# - orgao_uge_tipo_administracao_codigo: Código do tipo de administração
# - ug_executora_codigo: Código da UG executora
# - orgao_uge_codigo: Código do órgão UGE
# =============================================================================
...
```{r anexo_06_transf}
# RREO ANEXO 6 - TRANSFERÊNCIAS E SUBSÍDIOS - CÓDIGO DIRETO
# Script direto sem função - apenas totais


# =============================================================================
# FILTROS E PREPARAÇÃO DOS DADOS
# =============================================================================

# FILTROS DIRETOS NA DATAFRAME
dados_filtrados <- dados_despesa %>%
    filter(orgao_uge_orcam_fiscal_s_n  == "PERTENCE" ,
           !is.na(mes_lancamento),
           mes_lancamento != "",
           # Remover célula com data de execução do relatório TG
           ...
           # Manter apenas meses válidos (formato MMM/YYYY)
           grepl("^[A-Z]{3}/\\d{4}$", mes_lancamento),
           # FILTRO: Apenas órgãos que pertencem ao orçamento fiscal
           orgao_uge_orcam_fiscal_s_n == "PERTENCE",
           # FILTRO: Apenas pagamentos totais (exercício e RAP)
           item_informacao_codigo == 23)

# LISTA DE AÇÕES DE TRANSFERÊNCIAS CONSTITUCIONAIS E LEGAIS
acoes_transferencias <- c(
    "0044", "0045", "0046", "0050", "0051", "00D0", "00G6", "00H6", 
    "0169", "0223", "0369", "0546", "0547", "0999", "099B", "0A53", 
    "0C03", "0E25", "0E36", "00PX", "0C33", "006M", "00QR", "00RX", 
    "00S3", "00S7", "00S8", "00SE", "00SB", "00UH"
)

# LISTA DE AÇÕES DE SUBSÍDIOS
acoes_subsidios <- c(
    "0021", "0029", "0030", "0031", "0061", "006A", "0427", "0534", 
    "0A81", "0A84", "2130", "0062", "00DD", "00S5", "20GI", "006C", 
    "00ED", "00EE", "00SG"
)

# =============================================================================
# FUNÇÃO PARA ORDENAR MESES
# =============================================================================
...
# Obter meses únicos e ordenar cronologicamente
meses_unicos <- unique(dados_filtrados$mes_lancamento)
meses <- ordenar_meses(meses_unicos)
...
# Estrutura do relatório - APENAS TOTAIS
relatorio_transferencias <- data.frame(
    elemento = c(
        "TOTAL TRANSFERÊNCIAS CONSTITUCIONAIS E LEGAIS",
        "TOTAL SUBSÍDIOS E SUBVENÇÕES",
        "TOTAL GERAL (TRANSFERÊNCIAS + SUBSÍDIOS)"
    ),
    stringsAsFactors = FALSE
)
...
for (mes in meses) {
    ...
    # Filtrar por mês
    despesa_mes <- dados_filtrados[dados_filtrados$mes_lancamento == mes, ]
    
    # Total Transferências Constitucionais (todas as 30 ações)
    total_transferencias <- sum(despesa_mes[despesa_mes$acao_governo_codigo %in% acoes_transferencias, "saldo_r_item_informacao"], na.rm = TRUE)
    
    # Total Subsídios (todas as 19 ações)
    total_subsidios <- sum(despesa_mes[despesa_mes$acao_governo_codigo %in% acoes_subsidios, "saldo_r_item_informacao"], na.rm = TRUE)
    
    # Total Geral
    total_geral <- total_transferencias + total_subsidios
    
    # Adicionar ao relatório
    relatorio_transferencias[[mes]] <- c(
...
# Identificar colunas com valores numéricos
colunas_valores <- names(relatorio_transferencias)[names(relatorio_transferencias) != "elemento"]

datatable_transferencias <- DT::datatable(
...
for (i in 2:ncol(relatorio_transferencias)) {
    if (is.numeric(relatorio_transferencias[[i]])) {
        datatable_transferencias <- datatable_transferencias %>% 
            formatRound(i, 2, mark = ".", dec.mark = ",")
    }
}
...
# Mostrar o resultado
datatable_transferencias

# Para ver apenas o dataframe sem formatação:
# View(relatorio_transferencias)

# =============================================================================
# RESUMO DO QUE FOI CALCULADO:
# - 30 ações de transferências constitucionais e legais
# - 19 ações de subsídios e subvenções  
# - Filtro: orgao_uge_orcam_fiscal_s_n == "PERTENCE"
# - Filtro: item_informacao_codigo == 56 (Pagamentos Totais)
# =============================================================================
```

```{r rreo_anexo_6_disponibilidade}
# RREO ANEXO 6 - DISPONIBILIDADES - CÓDIGO SIMPLIFICADO
# Utiliza dcl_2a para capturar caixa e equivalentes de caixa
...
# FILTROS DIRETOS NA DATAFRAME - EXCLUINDO FUNDOS CORRETAMENTE
dados_filtrados <- tg_RPN_disponibilidades %>%
    filter(!is.na(mes_lancamento),
           mes_lancamento != "",
           # Remover célula com data de execução do relatório TG
           !grepl("^\\d{1,2}/\\d{1,2}/\\d{4}$", mes_lancamento),
           # Manter apenas meses válidos (formato MMM/YYYY)
           grepl("^[A-Z]{3}/\\d{4}$", mes_lancamento),
           # FILTRO CRUCIAL: Excluir Fundos (Tipo Administração diferente de 7)
           orgao_uge_tipo_administracao_codigo != 7,
           # FILTRO: Fontes específicas conforme TG
           fonte_recursos_codigo %in% c("77", "90", "490", "491"))
...
# Estrutura do relatório - FONTES ESPECÍFICAS
relatorio_disponibilidades <- data.frame(
    elemento = c(
        "RECURSOS A CLASSIFICAR (Fontes 77 + 490)",
        "RECURSOS DIVERSOS (Fontes 90 + 491)",
        "TOTAL DISPONIBILIDADES (Fontes Específicas)"
    ),
    stringsAsFactors = FALSE
)
...
for (mes in meses) {
    ...
    # Filtrar por mês
    disponib_mes <- dados_filtrados[dados_filtrados$mes_lancamento == mes, ]
    
    # Recursos a Classificar (Fontes 77 e 490)
    recursos_classificar <- sum(disponib_mes[disponib_mes$fonte_recursos_codigo %in% c("77", "490"), "saldo_r_conta_contabil"], na.rm = TRUE)
    
    # Recursos Diversos (Fontes 90 e 491)
    recursos_diversos <- sum(disponib_mes[disponib_mes$fonte_recursos_codigo %in% c("90", "491"), "saldo_r_conta_contabil"], na.rm = TRUE)
    
    # Total de todas as fontes específicas
    total_disponibilidades <- recursos_classificar + recursos_diversos
    
    # Adicionar ao relatório
    relatorio_disponibilidades[[mes]] <- c(
...
# Identificar colunas com valores numéricos
colunas_valores <- names(relatorio_disponibilidades)[names(relatorio_disponibilidades) != "elemento"]

datatable_disponibilidades <- DT::datatable(
    relatorio_disponibilidades,
    options = list(
        pageLength = 10,
        scrollX = TRUE,
        dom = 't',
...
# Aplicar formatação apenas em colunas numéricas
for (i in 2:ncol(relatorio_disponibilidades)) {
    if (is.numeric(relatorio_disponibilidades[[i]])) {
        datatable_disponibilidades <- datatable_disponibilidades %>% 
            formatRound(i, 2, mark = ".", dec.mark = ",")
    }
}
...
# Mostrar o resultado
datatable_disponibilidades

# Para ver apenas o dataframe sem formatação:
# View(relatorio_disponibilidades)

# =============================================================================
# RESUMO DO QUE FOI CALCULADO:
# - Dataframe: tg_RPN_disponibilidades
# - FILTRO CRUCIAL: orgao_uge_tipo_administracao_codigo != 7 (EXCLUIR FUNDOS)
# - Fontes: 77, 490 (Recursos a Classificar) e 90, 491 (Recursos Diversos)
# - Contas: 53 contas específicas já filtradas no TG
# - Filtros TG: Orçam. Fiscal = PERTENCE
# =============================================================================
```
...
```{r processar-dotacao}
# Processar dados para Dotação Atualizada (Item 12)
...
datatable(
  ...
  caption = "Tabela 02 - Dotação Atualizada por Tipo de Administração"
) %>%
  formatRound(
