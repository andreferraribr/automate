...
dados_receita <- read_excel(str_c(mes_pasta,"tg_receita.xlsx")) %>% 
  clean_names() %>% 
...
# RP Anexo 07
dados_rp_anexo_07 <- read_excel(str_c(mes_pasta,"tg_rp_anexo_07.xlsx")) %>% 
  clean_names()
...
# RP Anexo 12
dados_rp_anexo_12 <- read_excel(str_c(mes_pasta,"tg_rp_anexo_12.xlsx")) %>% 
  clean_names()

# RP Anexo 08
dados_rp_anexo_08 <- read_excel(str_c(mes_pasta,"tg_rp_anexo_08.xlsx")) %>% 
  clean_names()


dados_rp_anexo_03_rcl <- read_excel(str_c(mes_pasta,"tg_rp_anexo_03_rcl.xlsx")) %>% 
  clean_names()
...
# Buscar todos os arquivos de despesa
arq_tabela_04<- list.files(path = mes_pasta, pattern = 'tabela_04', full.names = TRUE)
...
dados_tabela_04_filtro_rreo <- read_excel(str_c(mes_pasta,"tg_rreo_tabela_04_filtro_rreo.xlsx")) %>% 
  clean_names() %>% 
  mutate(ug = "demais")
...
dados_ndd <- read_excel(str_c(mes_pasta,"tg_anexo_06_rreo_despesas_primarias_ndd.xlsx")) %>% 
  clean_names()

# Dados Categoria/Grupo
dados_categoria_grupo <- read_excel(str_c(mes_pasta,"tg_anexo_06_rreo_despesas_primarias_categoria_grupo.xlsx")) %>% 
  clean_names()
...
# RPN - Juros
tg_RPN_juros <- read_excel(str_c(mes_pasta,"tg_RPN_juros.xlsx")) %>% 
  clean_names()
dados_juros <- tg_RPN_juros
dados_juros$mes_lancamento %>% unique() %>% fct_inorder()
# RPN - Disponibilidades
tg_RPN_disponibilidades <- read_excel("data/tg_julho/tg_RPN_disponibilidades.xlsx") %>% 
  clean_names()
tg_RPN_disponibilidades$mes_lancamento %>% unique() %>% fct_inorder()

dados_ndd <- read_excel(str_c(mes_pasta,"tg_anexo_06_rreo_despesas_primarias_ndd.xlsx"))%>% clean_names()

dcl_1 <- read_excel(str_c(mes_pasta,"dcl_1.xlsx")) %>% clean_names()


dcl_2a <- read_excel(str_c(mes_pasta,"dcl_2a.xlsx")) %>% clean_names()
...
agrupar_e_combinar_dados_tipo_adm <- function(df, criterios) {
  ...
  map_df(names(criterios), function(categoria) {
    ...
    df %>%
      filter(condicao_expr) %>%
      group_by(mes_lancamento, orgao_uge_tipo_administracao_nome,item_informacao_nome, item_informacao_codigo) %>%
      summarise(
        saldo_r_item_informacao = sum(saldo_r_item_informacao, na.rm = TRUE), 
        .groups = "drop"
      ) %>%
...
tabela_02A_dotacao <- agrupar_e_combinar_dados_tipo_adm(df = dados_despesa %>% 
    filter( grupo_despesa_codigo_grupo == 1,  orgao_uge_orcam_fiscal_s_n == "PERTENCE",   mes_lancamento == mes_filtro,   item_informacao_codigo == "13"
    ),
...
DT_02A_dotacao <-  tabela_02A_dotacao %>% separate(categoria, into = c("ordem", "nome"), sep = 2, remove = FALSE) %>% group_by(orgao_uge_tipo_administracao_nome, ordem, nome) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao, na.rm = TRUE))%>%
  pivot_wider(
...
datatable(
  ...
  formatRound(
    which(sapply(DT_02A_dotacao, is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
...
# Processar dados para Despesas Liquidadas (Item 28)

tabela_02A_liquidada <- agrupar_e_combinar_dados_tipo_adm(df = dados_despesa %>% 
...
# Pivotar dados para visualização
DT_liquidadas_02A <-  tabela_02A_liquidada %>% separate(categoria, into = c("ordem", "nome"), sep = 2, remove = FALSE) %>% group_by(orgao_uge_tipo_administracao_nome, ordem, nome) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao, na.rm = TRUE))%>%
  pivot_wider(
    names_from = orgao_uge_tipo_administracao_nome,
    values_from = saldo_r_item_informacao,
    values_fill = 0
...
datatable(
  ...
  formatRound(
    which(sapply(DT_liquidadas_02A, is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
