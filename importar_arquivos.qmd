...
desmembrar_codigo <- function(codigo) {
  ...
  mes_lancamento <- str_sub(codigo, 1, 8)  # Ex.: mes_filtro
  ...
  if (!str_detect(mes_lancamento, "^[A-Z]{3}/\\d{4}$")) {
    stop("Formato de mes_lancamento inválido. Deve ser MMM/AAAA (ex.: JUN/2025)")
  }
...
arq_despesa <- list.files( pattern='tg_despesa')
...
dados_receita <- read_excel("tg_receita.xlsx") %>% clean_names() %>% filter(orgao_uge_orcam_fiscal_s_n == "PERTENCE")
...
dados_rp_anexo_07 <- read_excel("tg_rp_anexo_07.xlsx") %>% clean_names()
...
dados_rp_anexo_12 <- read_excel("tg_rp_anexo_12.xlsx") %>% clean_names()

# importar as contas contabeis para a tabela 04

dados_tabela_04 <- read_excel("dados_conta_contabil_tabela_04_claude_conta_contabil.xlsx") %>% clean_names()

dados_tabela_04_170600 <- read_excel("dados_conta_contabil_tabela_04_claude_conta_contabil_170600.xlsx") %>% clean_names() %>% mutate(ug = "170600")

dados_tabela_04_filtro_rreo <- read_excel("dados_conta_contabil_tabela_04_claude_conta_contabil_filtro_rreo.xlsx") %>% clean_names() %>% mutate(ug = "demais")

dados_ndd <- read_excel("tg_anexo_06_rreo_despesas_primarias_ndd.xlsx")%>% clean_names()

dados_categoria_grupo <- read_excel("tg_anexo_06_rreo_despesas_primarias_categoria_grupo.xlsx")%>% clean_names()
...
tg_RPN_juros <- read_excel("tg_RPN_juros.xlsx") %>% clean_names()

tg_RPN_disponibilidades <- read_excel("tg_RPN_disponibilidades.xlsx") %>% clean_names()
...
dcl_1 <- read_excel("dcl_1.xlsx") %>% clean_names()


dcl_2a <- read_excel("dcl_2a.xlsx") %>% clean_names()

dcl_2b <- read_excel("dcl_2b.xlsx") %>% clean_names()
```

```{r dt_mde}


anexo_08 <- agrupar_e_combinar_dados(dados_despesa, criterios_educacao, "JUN/2025REOA08despesas")
...
datatable (dados_rp_anexo_08%>%
  filter(mes_lancamento == mes_filtro,  !elemento_despesa_codigo %in% c('01', '03') &
...
dados_receita %>% filter(startsWith(natureza_receita_codigo_completo , '711')| startsWith(natureza_receita_codigo_completo , '111'), mes_lancamento == mes_filtro, item_informacao_codigo == 3) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao))
...
rli_transf <- dados_despesa %>% filter( item_informacao_codigo == "56", mes_lancamento == mes_filtro ) %>% mutate(transferencia = case_when(
...
rli_maio <- rli_mensal %>% filter(mes_lancamento == mes_filtro)  %>% summarise(minimo = sum(rli))
...
complementacao_fundeb <- dados_despesa %>% filter(item_informacao_codigo == 25, mes_lancamento == mes_filtro, fonte_recursos_codigo %in% c('000', '008' ,'014', '130'), acao_governo_codigo %in% c ('0E36', '00SB' ) )  %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao)*0.3)
...
despesas_saude_demais <- as.numeric( dados_despesa %>% filter(mes_lancamento == mes_filtro, iduso_codigo == 6, item_informacao_codigo == 25, subfuncao_governo_codigo %notin% c(301, 302, 303, 304, 305, 306 )) %>% summarise(saldo_r_item_informacao=sum(saldo_r_item_informacao)))
...
despesas_saude <- dados_despesa %>% filter(mes_lancamento == mes_filtro, iduso_codigo == 6, item_informacao_codigo == 25) %>% group_by(subfuncao_governo_codigo, subfuncao_governo_nome, categoria_economica_despesa_codigo,categoria_economica_despesa_nome, modalidade_aplicacao_codigo)%>% summarise(saldo_r_item_informacao=sum(saldo_r_item_informacao)) 
...
despesa_saude_nao_computadas <-  dados_despesa %>% filter(mes_lancamento == mes_filtro, iduso_codigo != 6, item_informacao_codigo == 25,uo_orgao_superior_codigo == "36000", unidade_orcamentaria_codigo != "74202" )%>% group_by(subfuncao_governo_codigo, subfuncao_governo_nome, categoria_economica_despesa_codigo,categoria_economica_despesa_nome, modalidade_aplicacao_codigo) %>%  summarise(saldo_r_item_informacao=sum(saldo_r_item_informacao)) 
...
dados_receita %>% filter(fonte_recursos_codigo %notin% c('00', '000') & (natureza_receita_codigo_completo %in% c('12105111', '12105121', '12105131', '12105141', '12150411', '12150412', '12150421', '12150431', '12150441') | startsWith(natureza_receita_codigo_completo, '121911')), unidade_orcamentaria_codigo != 73901, item_informacao_codigo == 5, mes_lancamento == mes_filtro ) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao, na.rm = TRUE))
```
...
criterios_despesas_anexo3_rcl <- list(
  ...
  # DEDUÇÕES  
  `02  Deduções` = list(
    criterio = "acao_governo_codigo %in% c('0E36', '00SB') | 
           ...
           (programa_governo_codigo %in% c('0903', '2030', '2080') & 
            ...
            acao_governo_codigo %in% c('0044', '0045', '0046', '0050', '0051', '00H6', '006M', '00G6', '0169', '0223', '0369', '0546', '0547', '0999', '099B', '0A53', '0C03', '0C33', '0E25', '0E36', '00PX', '00QR', '00S3', '00S7', '00S8', '00SE', '00RX', '00UH'))"
  )
)
...
DT_despesas_anexo3_rcl %>% filter(ordem == "02") %>% summarise(saldo = sum(`DESPESAS PAGAS`)) 
...
```{r}
datatable (dados_despesa %>% filter(mes_lancamento == "JUN/2025", item_informacao_codigo == 28, acao_governo_codigo %in% c('0E36', '00SB') | 
           grepl('^28846090900RX', programa_governo_codigo) | 
           (programa_governo_codigo %in% c('0903', '2030', '2080') & 
            modalidade_aplicacao_codigo %in% c(30, 31, 32, 35, 36, 40, 41, 42, 45, 46) & 
            acao_governo_codigo %in% c('0044', '0045', '0046', '0050', '0051', '00H6', '006M', '00G6', '0169', '0223', '0369', '0546', '0547', '0999', '099B', '0A53', '0C03', '0C33', '0E25', '0E36', '00PX', '00QR', '00S3', '00S7', '00S8', '00SE', '00RX', '00UH')) ) %>% group_by(modalidade_aplicacao_codigo, programa_governo_codigo, acao_governo_codigo) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao)))%>%
  formatRound(c('saldo_r_item_informacao'), 2, mark = ".", dec.mark = ",")


write.csv(dados_despesa %>% filter(mes_lancamento == "JUN/2025", item_informacao_codigo == 28, acao_governo_codigo %in% c('0E36', '00SB') | 
           grepl('^28846090900RX', programa_governo_codigo) | 
           (programa_governo_codigo %in% c('0903', '2030', '2080') & 
            modalidade_aplicacao_codigo %in% c(30, 31, 32, 35, 36, 40, 41, 42, 45, 46) & 
            acao_governo_codigo %in% c('0044', '0045', '0046', '0050', '0051', '00H6', '006M', '00G6', '0169', '0223', '0369', '0546', '0547', '0999', '099B', '0A53', '0C03', '0C33', '0E25', '0E36', '00PX', '00QR', '00S3', '00S7', '00S8', '00SE', '00RX', '00UH')) ) %>% group_by(modalidade_aplicacao_codigo, programa_governo_codigo, acao_governo_codigo) %>% summarise(saldo_r_item_informacao = sum(saldo_r_item_informacao)), "code.csv")
```
