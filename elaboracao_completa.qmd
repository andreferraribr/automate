...
datatable(
 rbind( 
   ...
   dados_receita %>% 
     ...
     summarise(receita_liquida = sum(receita_orcamentaria_liquida, na.rm = TRUE)),
...
datatable(dados_despesa %>%
 filter( !(elemento_despesa_codigo %in% c('76', '77') ) |!(fonte_recursos_codigo == '443' ) | !(subfuncao_governo_codigo %in% c('841', '842', '843', '844', '845', '846'))) %>%
  group_by(categoria_economica_despesa_codigo, categoria_economica_despesa_nome, grupo_despesa_codigo_grupo, grupo_despesa_nome) %>% summarise(despesas_empenhadas = sum(despesas_empenhadas, na.rm = TRUE) )  %>% adorn_totals("row"), rownames = FALSE,
...
tabela(dados_despesa %>% group_by(uge_uf_sigla, uge_uf_nome, programa_governo_codigo, programa_governo_nome) %>% summarise(despesas_empenhadas = sum(despesas_empenhadas, na.rm = TRUE)), "despesas_empenhadas")
```

### Despesa por UF e Ação

```{r}
tabela(dados_despesa %>% group_by(uge_uf_sigla, uge_uf_nome, acao_governo_codigo, acao_governo_nome) %>% summarise(despesas_empenhadas = sum(despesas_empenhadas, na.rm = TRUE)), "despesas_empenhadas")
```

### Despesa com irrigação

```{r}
tabela(dados_despesa %>% filter(acao_governo_codigo == "60") %>% group_by(uge_uf_sigla, uge_uf_nome) %>% summarise(despesas_empenhadas = sum(despesas_empenhadas, na.rm = TRUE)), "despesas_empenhadas")
```
...
tabela(anexo_01 , "despesas_empenhadas")
```
...
#   summarise_at(c(  "RECEITA ORCAMENTARIA (LIQUIDA)" ,"PREVISAO INICIAL DA RECEITA"  ,  "PREVISAO ATUALIZADA DA RECEITA"
#                  ,"DEDUCOES DA RECEITA" ), sum, na.rm = TRUE),
#   
#    dados_receita %>% 
#     head(-1) %>% 
#     clean_names() %>%
...
#      "Origem Código",
#      "Origem nome",
#      "RECEITA ORCAMENTARIA (LIQUIDA)" ,"PREVISAO INICIAL DA RECEITA"  ,  "PREVISAO ATUALIZADA DA RECEITA"
#                  ,"DEDUCOES DA RECEITA")
#  )
...
```{r}
tabela(dados_despesa %>% filter(resultado_eof_codigo == 6) %>% group_by(orgao_uge_orgao_maximo_codigo, orgao_uge_orgao_maximo_nome) %>% summarise(despesas_empenhadas = sum(despesas_empenhadas, na.rm = TRUE)), "despesas_empenhadas")
```
...
dados_despesa%>%
    ...
    summarise(across(c(despesas_empenhadas), \(x) sum(x, na.rm = TRUE))) %>% kable()
...
(datatable(dados_anexo_12_rp %>% group_by(mes_lancamento) %>% filter(iduso_codigo == 6, fonte_recursos_codigo %notin% c("45", "79"), grupo_despesa_codigo_grupo %notin% c(2,6), programa_governo_codigo %notin% c("0089" , "2019")) %>% summarise(restos_a_pagar_inscritos_proc_e_n_proc = sum(restos_a_pagar_inscritos_proc_e_n_proc, na.rm = TRUE)/1000)))

tabela((dados_anexo_12_rp %>% group_by(ne_c_cor_ano_emissao) %>% filter( fonte_recursos_codigo %notin% c("45", "79"), programa_governo_codigo %notin% c("0089" , "2019")) %>% summarise(valor = sum(restos_a_pagar_inscritos_proc_e_n_proc, na.rm = TRUE))),"valor" )
