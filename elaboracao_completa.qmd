...
receitas_primarias_rreo <- function(dados_receita) {
    ...
    dados <- dados_receita %>%
        filter(
            ...
            # Adicionar filtro para receita líquida para maior precisão
    ...
    # Calcular valores para cada mês com maior precisão
    for (mes in meses_ordenados) {
        ...
        # Aplicações Financeiras - usar código mais específico
        aplicacoes <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                  dados_mes$nre2_origem_receita_codigo_origem == 3 & 
                                  startsWith(dados_mes$natureza_receita_codigo_completo, "132"), "saldo_r_item_informacao"], na.rm = TRUE)
        ...
        demais <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                              dados_mes$nre2_origem_receita_codigo_origem %in% c(4, 5, 6, 8, 9), "saldo_r_item_informacao"], na.rm = TRUE)
        
        # Outras Receitas Financeiras - códigos específicos conforme metodologia
        outras_financeiras <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 1 & 
                                          dados_mes$natureza_receita_codigo_completo %in% c("164011011", "192012012", "199011111", "190992992", "164003131"), "saldo_r_item_informacao"], na.rm = TRUE)
        
        receitas_correntes_restantes <- correntes_total - impostos - contribuicoes - patrimonial - transferencias - demais
        ...
        outras_capital_nao_primarias <- sum(dados_mes[dados_mes$nre1_categoria_economica_codigo == 2 & 
                                                    dados_mes$natureza_receita_codigo_completo %in% c("292292", "293293", "294294"), "saldo_r_item_informacao"], na.rm = TRUE)
