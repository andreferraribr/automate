...
# SQL: "a11"."IN_ISF_LANC" in ('P') E filtros complexos
demais_dividas_contratuais <- dcl_2a %>%
  filter(
    (is.na(c_con_isf) | c_con_isf %in% c("P", "p", "S", "s")),
    (
      # Contas específicas disponíveis
      conta_contabil_numero %in% c(222210200, 212210300, 212110303, 212310201, 212310202, 212410201, 222310101, 222310102, 222410101, 217710101, 227710101) |
      # Padrões similares de contas contratuais
      substr(as.character(conta_contabil_numero), 1, 4) %in% c("2222", "2121", "2123", "2124", "2177", "2277") |
      # Exceção específica do SQL
      (conta_contabil_numero == 227310401 & ug_executora_codigo != "170512")
    )
  ) %>%
  summarise(valor = sum(saldo_r_conta_contabil, na.rm = TRUE)) %>%
  pull(valor)
...
aplicacoes_fundos_3 <- dcl_2a %>%
  filter(orgao_uge_tipo_administracao_codigo == 7,
         ...
         (str_starts(as.character(conta_contabil_numero), "1111102") |
          ...
          (str_starts(as.character(conta_contabil_numero), "1111119") & ug_executora_codigo != "380916") |
          (c_con_classe_1_codigo == 1 & c_con_grupo_2_codigo_grupo == 2 & c_con_subgrupo_3_codigo_subgrupo %in% c(2, 3, 4, 5)))) %>%
  summarise(valor = sum(saldo_r_conta_contabil, na.rm = TRUE)) %>%
  pull(valor)
