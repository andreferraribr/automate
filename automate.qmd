...
consolidar_criterios <- function() {
  ...
  ano <- str_extract(mes_recente, "\\d{4}")
  mes_num <- case_when(
    grepl("JAN", mes_recente) ~ "01",
    grepl("FEV", mes_recente) ~ "02",
    grepl("MAR", mes_recente) ~ "03",
    grepl("ABR", mes_recente) ~ "04",
    grepl("MAI", mes_recente) ~ "05",
    grepl("JUN", mes_recente) ~ "06",
    grepl("JUL", mes_recente) ~ "07",
    grepl("AGO", mes_recente) ~ "08",
    grepl("SET", mes_recente) ~ "09",
    grepl("OUT", mes_recente) ~ "10",
    grepl("NOV", mes_recente) ~ "11",
    grepl("DEZ", mes_recente) ~ "12",
    TRUE ~ "00"
  )
...
datatable(aplicar_criterios(dados_receita, criterios_rreo_A03_rcl_receitas))
...
dcl_2b_agrupado <- agrupar_e_combinar_dados_conta_contabil(
       df =  dcl_2b,
        criterios = criterios_dcl_2b
      )
 
 datatable(
  processar_dt_criterios_conta_contabil( dcl_2b_agrupado),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "dcl_2a_agrupado"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios_conta_contabil( dcl_2b_agrupado), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
 ...
 dcl_2a_agrupado <- agrupar_e_combinar_dados_conta_contabil(
       df =  dcl_2a,
        criterios = criterios_dcl_2a
      )
 
 datatable(
  processar_dt_criterios_conta_contabil( dcl_2a_agrupado),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ...
  formatRound(
    which(sapply(processar_dt_criterios_conta_contabil( dcl_2a_agrupado), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
 ...
 dcl_1_agrupado <- agrupar_e_combinar_dados_conta_contabil(
       df =  dcl_1,
        criterios = criterios_dcl_1
      )
 
 datatable(
  processar_dt_criterios_conta_contabil( dcl_1_agrupado),
  options = list(
    pageLength = 10,
    scrollX = TRUE,
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Portuguese-Brasil.json')
  ),
  caption = "dcl_1_agrupado"
) %>%
  formatRound(
    which(sapply(processar_dt_criterios_conta_contabil( dcl_1_agrupado), is.numeric)), 
    digits = 2, 
    mark = ".", 
    dec.mark = ","
  )
